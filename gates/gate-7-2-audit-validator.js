...
/**
 * Gate 7-2: Full Build Polish Audit Validator
 * Validates full-polish-findings.json has valid structure.
 */
...
function validate(phaseDef, qualityGate) {
  const findingsPath = path.join(process.cwd(), 'reference', 'full-polish-findings.json');

  if (!fs.existsSync(findingsPath)) {
    qualityGate.passed = false;
    qualityGate.criticalFailures.push('reference/full-polish-findings.json not found');
    return qualityGate;
  }
